...
public class NonConformitaDaoImpl /*extends AbstractDao<Integer, NonConformita> */ implements NonConformitaDao {
    ...
    public List<ArrayList> getOnlyNc(){
        ...
        for(int year=2017; year<= Calendar.getInstance().get(Calendar.YEAR); year++){
            ...
            for(int i=0; i<12; i++){
                ...
                tmp.add(Restrictions.ge("dataInizio", di)).add(Restrictions.le("dataInizio", df)).add(Restrictions.isNotNull("dataInizio")).add(Restrictions.isNotNull("azioneContenimento"));
