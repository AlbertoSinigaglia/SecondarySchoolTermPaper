                        ...
                        <div class="w3-container w3-card w3-white w3-margin-bottom">
                            <div class="w3-row">
                                <div class="w3-col l10 m8 s12">
                                    <h2 class="w3-text-grey w3-padding-16"><i class="fa fa-suitcase fa-fw w3-margin-right w3-xxlarge w3-text-teal"></i>NC nr:<b> ${nc.nr}</b></h2>
                                </div>
<c:if test="${nc.dataFine!=null}">
...
<c:if test="${nc.dataFine==null&&nc.dataInizio!=null&&nc.azioneContenimento==null}">
                            ...
                            <hr>
                            <div class="w3-container">
                                <h6 class="w3-text-teal"><span class="w3-text-black"><b>Aperta il:</b></span> <i class="fa fa-calendar fa-fw"></i>${nc.realDataInizio()}</h6>
                                    <c:if test="${nc.dataFine!=null}">
                                    <h6 class="w3-text-teal"><span class="w3-text-black"><b>Chiusa il:</b></span> <i class="fa fa-calendar fa-fw"></i>${nc.realDataFine()}</h6>
                                    </c:if>
                                    <c:if test="${nc.dipendenteRisc!=null}">
                                    <h6 class="w3-text-teal"><span class="w3-text-black" onclick="location.href='infoDip?email=${nc.dipendenteRisc.email}'"><b>Aperta da (Dipendente):</b> ${nc.dipendenteRisc.cognome} ${nc.dipendenteRisc.nome}</span></h6>
                                </c:if>
                                <c:if test="${nc.cliente!=null}">
                                    <h6 class="w3-text-teal"><span class="w3-text-black" onclick="location.href='infoCli?piva='${nc.cliente.piva}'"><b>Aperta da (Cliente):</b> ${nc.cliente.azienda} (P.IVA: ${nc.cliente.piva})</span></h6>
                                </c:if>
                                ...
                                <label style="margin-bottom:8px !important;">
                                    <c:forEach items="${nc.dipendentiGest}" var="dip">
                                        <p class="inline" onclick="location.href='infoDip?email=${dip.email}'">${dip.cognome} ${dip.nome}</p>
                                    </c:forEach>
                                </label>
                            ...
                            <div class="w3-row">
                                <button class="bottone" onclick="conferma()">Chiudi non conformit&agrave;</button></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <script>
            ...
            function conferma(){
                if(confirm("Attenzione: una volta chiusa, non sarà più possibile riaprire e modificare questa non conformità, procedere con la chiusura?")) location.href="chiudi?nr=${nc.nr}";
            }
        </script>
    </body>
</html>
